...
int http_tcp_listen_rx(HTTPSRV * p_srv)
{
	...
	int cfd = accept(p_srv->sfd, (struct sockaddr *)&caddr, &size);
	if(cfd < 0)
	{
	...
	HTTPCLN * p_cln = http_get_idle_cln(p_srv);
	if(p_cln == NULL)
	{
...
int http_srv_net_init(HTTPSRV * p_srv)
{
	...
	if(p_srv->sfd < 0)
	{
		printf("http_srv_net_init::socket err[%s]!!!\r\n", sys_os_get_socket_error());
		return -1;
	}
	...
	{
		printf("http_srv_net_init::bind tcp socket fail,err[%s]!!!\n", sys_os_get_socket_error());
	...
	{
		printf("rpc_srv_net_init::listen tcp socket fail,err[%s]!!!\r\n", sys_os_get_socket_error());
		closesocket(p_srv->sfd);
		return -1;
	}
