...
HT_API int http_line_parse(char * p_buf, int max_len, char sep_char, PPSN_CTX * p_ctx)
{
	...
	do {
		...
		snprintf(pHdrV->header, sizeof(pHdrV->header), "%.*s",
		         (int)(sizeof(pHdrV->header) - 1), word_buf);
...
HT_API BOOL http_ctt_parse(HTTPMSG * p_msg)
{
	...
	{		
		...
		else if (strcasecmp(pHdrV->header, "Content-Type") == 0)
		{
			...
			else if (strcasecmp(type_word, "application/soap+xml") == 0 || strcasecmp(type_word, "text/xml") == 0 || strcasecmp(type_word, "application/xml") == 0)
			{
				w_ctx_type = CTT_XML;
			}
            ...
            else if (strcasecmp(type_word, "multipart/form-data") == 0) 
            {
                w_ctx_type = CTT_FORM;
                
                // 解析boundary
                char *boundary = strstr(pHdrV->value_string, "boundary=");
                if (boundary) 
                {
                    strncpy(p_msg->boundary, boundary+9, sizeof(p_msg->boundary)-1);
                }
            }
