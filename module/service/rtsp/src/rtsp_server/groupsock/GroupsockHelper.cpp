...
int setupDatagramSocket(UsageEnvironment& env, Port port) {
  ...
  int reuse_flag = 1; //johnnyling 20121219
...
int makeSocketTcpNoDelay(int nSocket)
{
    int nSet = 1;
    
    if (setsockopt(nSocket, IPPROTO_TCP, TCP_NODELAY, (char*)&nSet, sizeof(int)))    
    {
        return -1;
    }
...
int setupStreamSocket(UsageEnvironment& env,
...
#if 0
  //johnnyling 20121219 added to avoid bind failed!!!
  struct linger so_linger;
  so_linger.l_onoff =1;
  so_linger.l_linger=3; //wait 3 seconds before close socket
  if((setsockopt(newSocket, SOL_SOCKET, SO_LINGER, (void *)&so_linger, sizeof(struct linger))) == -1) 
  {
  ...
  if (makeNonBlocking) {
    if (!net_makeSocketNonBlocking(newSocket)) {
...
Boolean badAddress(netAddressBits addr) {
...
#if 0
static void get_my_ifname(char *ifname, char *ip_addr)
{
    *ifname = '\0';

    if((is_network_interface_up("eth0") == 1) && net_get_ifaddr("eth0") != INADDR_NONE)
    {
        strcpy(ifname,"eth0");
    }
    else if(is_network_interface_up("wlan0") > 0 && net_get_ifaddr("wlan0") != INADDR_NONE)
    {
        strcpy(ifname, "wlan0");
    }
    if(!strlen(ifname))
    {
        strcpy(ifname,"eth0:0");
    }
}

static in_addr_t get_my_ipaddr(char *ip_addr)
{
    ...
    char ifname[256] = {'\0'};
    get_my_ifname(ifname, ip_addr);
    printf("**********%s**********\n", ifname);
    __my__ip = net_get_ifaddr(ifname);
    if(INADDR_NONE == __my__ip)
    {
        __my__ip = net_get_ifaddr("eth0:0");
    }
    return __my__ip;
}
#endif
...
#include <hostLib.h>
    ...
    SystemGetTimeofRun(&timeNow, NULL);
    unsigned seed = ourAddress^timeNow.tv_sec^timeNow.tv_usec;
    our_srandom(seed);
    #endif

    unsigned long int32 = get_my_ipaddr();
    ourAddress = int32;
  }
  return ourAddress;
}
...
// For Windoze, we need to implement our own SystemGetTimeofRun()
#if !defined(_WIN32_WCE)
#include <sys/timeb.h>
#endif
...
int SystemGetTimeofRun(struct timeval* tp, int* /*tz*/) {
#if defined(_WIN32_WCE)
