...
#define MAX_SCHEDULER_GRANULARITY 40000 // 40 microseconds: We will return to the event loop at least this often
...
#define BASIC_TASK_SCHEDULER_TIME 100
unsigned g_nBasicTaskScheduler = 0;

void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
  fd_set readSet = fReadSet; // make a copy for this select() call
  fd_set writeSet = fWriteSet; // ditto
  fd_set exceptionSet = fExceptionSet; // ditto
  unsigned uMaxDelayTimeSec = 0;
  unsigned uMaxDelayTimeUsec = 0;  
  ...
  /* 为了最快速度把数据流发送出去,避免卡顿,不让该线程执行select操作让CPU */
  if ( (g_nBasicTaskScheduler < BASIC_TASK_SCHEDULER_TIME)
      && ((tv_timeToDelay.tv_sec == 0) && (tv_timeToDelay.tv_usec == 0)))
  {
      // Also handle any delayed event that may have come due.
      fDelayQueue.handleAlarm();
      g_nBasicTaskScheduler++;
      return;
  }

  g_nBasicTaskScheduler = 0;
  ...
  if (maxDelayTime > 0)
  {
      uMaxDelayTimeSec  = maxDelayTime/MILLION;
      uMaxDelayTimeUsec = maxDelayTime%MILLION;

      if ( ((unsigned)tv_timeToDelay.tv_sec > uMaxDelayTimeSec)
          || (((unsigned)tv_timeToDelay.tv_sec == uMaxDelayTimeSec)
                && ((unsigned)tv_timeToDelay.tv_usec > uMaxDelayTimeUsec)) )
      {
          tv_timeToDelay.tv_sec = uMaxDelayTimeSec;
          tv_timeToDelay.tv_usec = uMaxDelayTimeUsec;
      }
  }
