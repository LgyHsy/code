...
volatile int g_rtsp_client_count = 0;
...
RTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  do {
    ...
    return new RTSPServer(env, ourSocket, ourPort, authDatabase,
                      reclamationTestSeconds, MaxClientSocketNum);
  } while (0);
...
void RTSPServer::addServerMediaSession(ServerMediaSession* serverMediaSession) {
  ...
  for(i = 0; i < 5; i++)
  {
  	const char *otherStreamName = serverMediaSession->GetOtherStreamName(i);
  	if(strlen(otherStreamName))
  	{
  		existingSession = (ServerMediaSession*)(fServerMediaSessions->Add(otherStreamName, (void*)serverMediaSession));
  		__ERR("rtsp other name[%d]:%s, existingSession:%x\n", i, otherStreamName, existingSession);
  	}
  }
}
...
void RTSPServer::removeServerMediaSession(ServerMediaSession* serverMediaSession) {
  ...
  for(i=0;i<5;i++)
  {
  	...
  	if(strlen(otherStreamName))
  	{
  		fServerMediaSessions->Remove(otherStreamName);
  	}
  }  
...
char* RTSPServer
::rtspOtherURL(ServerMediaSession const* serverMediaSession, int clientSocket, const char *otherName) const {
  char* urlPrefix = rtspURLPrefix(clientSocket);
  char const* sessionName = otherName;
...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
  ...
  if (portNumHostOrder == 554 /* the default port number */) {
    ...
    sprintf(urlBuffer, "rtsp://%s/", our_inet_ntoa(ourAddress.sin_addr));  //PSIA/Streaming/channels/
  }
  else {
...
void RTSPServer::closeAllClientSession()
{
    std::list<RTSPClientSession*>::iterator Session;    
    RTSPClientSession* pstClientSession;
        
    for (Session = fClientSessionList.begin(); 
         Session != fClientSessionList.end();
         ++Session)
    {
        pstClientSession = (RTSPClientSession*)*Session;
        envir().taskScheduler().scheduleDelayedTask(0, 
                                    (TaskFunc*)pstClientSession->deleteClientSession,
                                    pstClientSession);
    }
...
RTSPServer::RTSPServer(UsageEnvironment& env,
  ...
  : Medium(env),
    ...
    fMaxClientSocketNum(MaxClientSocketNum), fServerRecvClientSocketNum(0)
{
...
void RTSPServer::incomingConnectionHandler(int serverSocket) {
  ...
  net_makeSocketNonBlocking(clientSocket);
  __INFO("accept connection from %s, sock = %d\n", our_inet_ntoa(clientAddr.sin_addr), clientSocket);
#ifdef DEBUG
  envirclientSocket << "accept()ed connection from " << our_inet_ntoa(clientAddr.sin_addr) << '\n';
#endif

  /* 限制最大客户端连接 */
  if (fServerRecvClientSocketNum >= fMaxClientSocketNum)
  {
      ::closeSocket(clientSocket);
      __ERR("CloseSocket clientSocket:%d\n\r", 
            clientSocket);      
      return;
  }

  fServerRecvClientSocketNum++;
  ...
  __INFO("New Client SessionId:%d \n\r", sessionId);
  pstClientSession = createNewClientSession(sessionId, clientSocket, clientAddr);
  
  __INFO("fClientSessionList Number:%d \n\r", fClientSessionList.size());
  fClientSessionList.push_back(pstClientSession);
}
...
#if 0   //todo
RTSP_Stream_Connect_Info_t gRTSP_Stream_Connect_Info;
void RTSP_StreamConnectInfo(RTSP_Stream_Connect_Info_t Stream_Connect_Info)
{
	AUX_DATA aux_data;
    memset(&aux_data, 0, sizeof(aux_data));

	aux_data.data_type = 222;
	memcpy(aux_data.data_buf, &Stream_Connect_Info, sizeof(Stream_Connect_Info));
...
::RTSPClientSession(RTSPServer& ourServer, unsigned sessionId, int clientSocket, struct sockaddr_in clientAddr)
  : fOurServer(ourServer), fOurSessionId(sessionId),
    ...
    fSessionCookie(NULL), fLivenessCheckTask(NULL), fClientSessionDeleting(False),
    fIsMulticast(False), fSessionIsActive(True), fStreamAfterSETUP(False),
    fTCPStreamIdCount(0), fSocketRecvZero(0), fSocketAuthPass(False), 
    fStreamIndex(-1), fCameraIndex(0), fNumStreamStates(0), fStreamStates(NULL) {
...
RTSPServer::RTSPClientSession::~RTSPClientSession() {
  ...
  __ERR("delete RTSP client session!!! fClientOutputSocket: %d, g_rtsp_client_count = %d\n", fClientOutputSocket, g_rtsp_client_count);
  fClientSessionDeleting = True;
  ...
  if (fOurServer.fServerRecvClientSocketNum > 0)
  {
      __ERR("###########del fServerRecvClientSocketNum\n\r");
      fOurServer.fServerRecvClientSocketNum--;
  }
  ...
  if (fOurServerMediaSession != NULL) {
	...
	//char *tmpstr = (char *)fOurServerMediaSession->streamName();
	//char *pClientStr = our_inet_ntoa(fClientAddr.sin_addr);
	//int i = 0;

#if 0   //todo
	if(!strncmp(tmpstr, "mpeg4cif", strlen("mpeg4cif")) || 
		!strncmp(tmpstr, "h264cif", strlen("h264cif")) ||
		!strncmp(tmpstr, "h265cif", strlen("h265cif")) ||
		!strncmp(tmpstr, "1", strlen("1")) || 
		!strncmp(tmpstr, "stream2", strlen("stream2")))
	{
		 gRTSP_Stream_Connect_Info.SubStreamInfo.ConnectCon--;
		 if(gRTSP_Stream_Connect_Info.SubStreamInfo.ConnectCon < 0)
			 gRTSP_Stream_Connect_Info.SubStreamInfo.ConnectCon = 0;
		 
		 //连接来自同一个设备 连接计数-1
		 for(i = 0; i < RTSP_MAX_VENC_CHN; i++)
		 {
			 tmpstr = gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Client;
			 if(!strncmp(tmpstr, pClientStr, strlen(pClientStr)))
			 {
				  gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Count--;
				  if(gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Count <= 0)
				  {
					  gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Count = 0;
					  memset(tmpstr, 0, sizeof(gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Client));
				  }
				  break;
			 }
		 }
	}
	else if(!strncmp(tmpstr, "mpeg4", strlen("mpeg4")) || 
		!strncmp(tmpstr, "h264", strlen("h264")) || 
		!strncmp(tmpstr, "h265", strlen("h265")) || 
		!strncmp(tmpstr, "0", strlen("0")) || 
		!strncmp(tmpstr, "stream1", strlen("stream1")))
	{
		 gRTSP_Stream_Connect_Info.MainStreamInfo.ConnectCon--;
		 if(gRTSP_Stream_Connect_Info.MainStreamInfo.ConnectCon < 0)
			 gRTSP_Stream_Connect_Info.MainStreamInfo.ConnectCon = 0;
		
		//连接来自同一个设备 连接计数-1
		for(i = 0; i < RTSP_MAX_VENC_CHN; i++)
		{
			tmpstr = gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Client;
			if(!strncmp(tmpstr, pClientStr, strlen(pClientStr)))
			{
				 gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Count--;
				 if(gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Count <= 0)
				 {
					 gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Count = 0;
					 memset(tmpstr, 0, sizeof(gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Client));
				 }
				 break;
			}
		}
	...
	RTSP_StreamConnectInfo(gRTSP_Stream_Connect_Info);
#endif
  ...
  fClientInputSocket = -1;
  g_rtsp_client_count--;
}

void RTSPServer::RTSPClientSession::deleteClientSession(RTSPClientSession* clientSession)
{    
    if (NULL != clientSession)
    {
        if (clientSession->fClientSessionDeleting == True)
        {
            return;
        }
        clientSession->fOurServer.fClientSessionList.remove(clientSession);
...
void RTSPServer::RTSPClientSession::handleRequestBytes(int newBytesRead) {
 // noteLiveness();
  ...
  if (newBytesRead < 0 || (unsigned)newBytesRead >= fRequestBufferBytesLeft) {
    ...
    __ERR("RTSPClientSession::handleRequestBytes() read %d new bytes (of %d); terminating connection!\n",
           newBytesRead, fRequestBufferBytesLeft);
    ...
    __ERR("RTSPClientSession::handleRequestBytes() Delete Client Session!\n");

    deleteClientSession(this);
    return;
  }

  if (newBytesRead > 0)	
  {
	//__INFO("RTSPClientSession::handleRequestBytes() read %d new bytes (of %d)\n",
    //       newBytesRead, fRequestBufferBytesLeft);
  }
  else
  {
      //newBytesRead is zero
      return;
  }
  ...
  //ptr[newBytesRead] = '\0';
  //if ((ptr[0] == 0x81) && (ptr[0] == 0xc9))
  //{
  //__INFO("RTSPClientSession[%p]::handleRequestBytes() read %d new bytes:%s\n", this, newBytesRead, ptr);
  //}
  ...
  if (parseRTSPRequestString((char*)fRequestBuffer, fRequestBytesAlreadySeen,
    ...
    {
        return;// we still need more data; subsequent reads will give it to us 
    }

    /* If cmd name is Invisible characters, clear requeset buff;
       Don't response error to client, and leading to client teardown */
    if ((cmdName[0] >= 0x81)
        && (cmdName[1] >= 0x81))
    {
        __ERR("cmdName:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x \n", 
            cmdName[0], cmdName[1], cmdName[2], cmdName[3],
            cmdName[4], cmdName[5], cmdName[6], cmdName[7]);
        __ERR("fRequestBuffer:%s \n", fRequestBuffer);

        resetRequestBuffer();
        return;
    }
    
  //  __INFO("======parseRTSPRequestString(%s) succeeded, cmdName \"%s\", urlPreSuffix \"%s\", urlSuffix \"%s\" SessionId \"%s\" fClientOutputSocket %d\n",
           //our_inet_ntoa(fClientAddr.sin_addr), cmdName, urlPreSuffix, urlSuffix, sessionIdStr, fClientOutputSocket);
    ...
    if (strcmp(cmdName, "OPTIONS") == 0) {
      ...
      handleCmd_DESCRIBE(cseq, urlSuffix, (char const*)fRequestBuffer);
		...
		char *real_url_Suffix=strstr(urlPreSuffix,"?username=");
		if(real_url_Suffix)
		{
			*real_url_Suffix='\0';
		}
		handleCmd_SETUP(cseq, urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
	       ...
	       || strcmp(cmdName, "SET_PARAMETER") == 0) {
	       	char *real_url_Suffix=strstr(urlSuffix,"?username=");
	       	if(real_url_Suffix)
	       	{
	       		*real_url_Suffix='\0';
	       	}
    ...
    __INFO("parseRTSPRequestString() failed\n");
    ...
    if (parseHTTPRequestString(cmdName, sizeof cmdName,
      ...
      __INFO("parseHTTPRequestString() succeeded, returning cmdName \"%s\", urlSuffix \"%s\", sessionCookie \"%s\", acceptStr \"%s\"\n",
	...
	handleHTTPCmd_notSupported();
      ...
      __INFO("parseHTTPRequestString() failed!\n");
      handleCmd_bad(cseq);
    }
  }

  //__INFO("sending response: %s", fResponseBuffer);
  //teardown send probability of segmentfault
...
::handleCmd_DESCRIBE(char const* cseq, char const* urlSuffix,
  ...
  //__ERR("urlSuffix = %s, fullRequestStr=%s\n", urlSuffix, fullRequestStr);
      
  do {
      if (!authenticationOK("DESCRIBE", cseq, urlSuffix, fullRequestStr))
      {
          __ERR("authentication Error!\n");
          fSocketAuthPass = False;
          break;
      }

      __INFO("authentication OK!\n");
      fSocketAuthPass = True;

    //remove the username and password from urlSuffix
    char *ptr = strstr(urlSuffix, "?username");
    if(ptr)
    {
        *ptr='\0';	
        __INFO("remove username and password new urlSuffix=%s\n", urlSuffix);
    }
    ...
    if(strcmp(rtspURL, urlSuffix))
    {
    	delete rtspURL;
    	rtspURL = fOurServer.rtspOtherURL(session, fClientInputSocket,urlSuffix);	
    }
...
::handleCmd_SETUP(char const* cseq,
  ...
  if (False == fSocketAuthPass)
  {
      fSessionIsActive = False;
      return;
  }
  
  ServerMediaSession* pMediaSession = fOurServer.lookupServerMediaSession("mpeg4");
  if(pMediaSession)
  	main_refcount = pMediaSession->referenceCount();
  pMediaSession = fOurServer.lookupServerMediaSession("mpeg4cif");
  if(pMediaSession)
  	sub_refcount = pMediaSession->referenceCount();
    ...
    if ( urlPreSuffix[0] != '\0' &&
         urlPreSuffix[0] == 'c' &&
         urlPreSuffix[1] == 'h' &&
         isdigit(urlPreSuffix[2]) &&
         isdigit(urlPreSuffix[3]) &&
         strstr(urlPreSuffix, "/") != NULL)
    {
        IsAnjMutiLens = 1;
    }
    ...
    if (IsAnjMutiLens == 1)
    {
        ...
        int chNum = -1;
        char chNumStr[3] = {0};
        chNumStr[0] = urlPreSuffix[2];
        chNumStr[1] = urlPreSuffix[3];
        chNum = atoi(chNumStr);

        fCameraIndex = chNum - 1;

        pSuffix = strstr(urlPreSuffix, "/");
        pSuffix++;
    }
    else
    {
        fCameraIndex = 0;
        pSuffix = urlPreSuffix;
    }

    if ( (strncmp(pSuffix, "mpeg4cif", strlen("mpeg4cif")) == 0) || 
            (strncmp(pSuffix, "h264cif", strlen("h264cif")) == 0) || 
            (strncmp(pSuffix, "h265cif", strlen("h265cif")) == 0) || 
            (strncmp(pSuffix, "1", strlen("1")) == 0) || 
            (strncmp(pSuffix, "stream2", strlen("stream2")) == 0) )
    {
        fStreamIndex = 1; //sub stream
    }
    else
    {
        fStreamIndex = 0; //main stream
    }
  
  __INFO("########## main count:%d, sub count:%d, urlPreSuffix:%s, urlSuffix:%s, fCameraIndex:%d, fStreamIndex:%d\n",
         main_refcount, sub_refcount, urlPreSuffix, urlSuffix, fCameraIndex, fStreamIndex);
  ...
  if (sms == NULL) 
  {
    if (fOurServerMediaSession == NULL) 
    {
      ...
      handleCmd_notFound(cseq);
      __ERR("111 sms Is Null And fOurServerMediaSession Is Not Null\n");      
    } 
    else 
    {
      // The client asked for a stream that doesn't exist, but using a stream id for a stream that does exist. Bad request:
      handleCmd_bad(cseq);
      __ERR("222 sms Is Null And fOurServerMediaSession Is Not Null\n");
    }
  ...
  else 
  {
    ...
    else if (sms != fOurServerMediaSession) 
    {
      ...
      handleCmd_bad(cseq);
      __ERR("333 sms And fOurServerMediaSession Is Not EQ\n");
      return;
    }
  }
  ...
  if (trackId != NULL && trackId[0] != '\0') { // normal case
    ...
    if (fNumStreamStates != 1) {
      handleCmd_bad(cseq);
      __ERR("555 fNumStreamStates(%d) is Not 1 \n", fNumStreamStates);            
      return;
    }
    streamNum = 0;
    subsession = fStreamStates[streamNum].subsession;
...
::handleCmd_withinSession(char const* cmdName,
  ...
  //__ERR("fullRequestStr: %s, urlPreSuffix: %s, urlSuffix: %s\n", fullRequestStr, urlPreSuffix, urlSuffix);  
  // This will either be:
  ...
  if (urlPreSuffix[0] == '\0' && urlSuffix[0] == '*' && urlSuffix[1] == '\0') {
    ...
    if (subsession == NULL) { // no such track!
      ...
      return;
  			...
  			fOurServerMediaSession->CheckOtherStreamName(urlSuffix) ||
  ...
  if (strcmp(cmdName, "TEARDOWN") == 0) {
    ...
    handleCmd_PLAY(subsession, cseq, fullRequestStr,urlPreSuffix);
...
::handleCmd_TEARDOWN(ServerMediaSubsession* /*subsession*/, char const* cseq) {
  ...
  //forceKeyframe when got rtsp command play
   char* rtspURL = fOurServer.rtspURL(fOurServerMediaSession, fClientInputSocket);
   __INFO("teardown stream, url: %s, client: %s\n", 
		  rtspURL, our_inet_ntoa(fClientAddr.sin_addr));   
   delete[] rtspURL;		
}
...
void RTSPServer::RTSPClientSession
  ::handleCmd_PLAY(ServerMediaSubsession* subsession, char const* cseq,
		   char const* fullRequestStr, char const *urlSuffix) {
  ...
  //__ERR("urlSuffix=%s, streamName: %s\n", urlSuffix, fOurServerMediaSession->streamName());
  
  if(strcmp(urlSuffix, fOurServerMediaSession->streamName()))
  {
  	delete rtspURL;
  	rtspURL = fOurServer.rtspOtherURL(fOurServerMediaSession, fClientInputSocket, urlSuffix);
  	rtspURLSize  = strlen(rtspURL);
  }

  //forceKeyframe when got rtsp command play
	__INFO("start one rtsp stream, url: %s, client: %s\n", 
		   rtspURL, our_inet_ntoa(fClientAddr.sin_addr));

	//为了兼容雄迈 等其他的NVR的视频连接请求信息 使用tmpstr = fOurServerMediaSession->streamName();
	//char *tmpstr = (char *)fOurServerMediaSession->streamName();
	//char *pClientStr = our_inet_ntoa(fClientAddr.sin_addr);
	unsigned int i = 0;
		...
		!strncmp(tmpstr, "stream2", strlen("stream2")))
	{
		...
		if(i == RTSP_MAX_VENC_CHN)
		{
			for(i = 0; i < RTSP_MAX_VENC_CHN; i++)
			{
				...
				if(!strlen(tmpstr))
				{
					strncpy(tmpstr, pClientStr, sizeof(gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Client));
					gRTSP_Stream_Connect_Info.SubStreamInfo.ClientInfo[i].Count++;
					break;
				}
			}
		...
		!strncmp(tmpstr, "stream1", strlen("stream1")))
	{
		...
		if(i == RTSP_MAX_VENC_CHN)
		{
			for(i = 0; i < RTSP_MAX_VENC_CHN; i++)
			{
				...
				if(!strlen(tmpstr))
				{
					strncpy(tmpstr, pClientStr, sizeof(gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Client));
					gRTSP_Stream_Connect_Info.MainStreamInfo.ClientInfo[i].Count++;
					break;
				}
			}
    ...
    /* 强制I帧 */
    anj_video_request_idr(fCameraIndex, fStreamIndex);

	if(fTCPStreamIdCount > 0)
  	{
        unsigned requestedSize = 64*1024;
        
  		if(1 == fStreamIndex) //sub stream
  		{
            requestedSize = 200*1024;
  		}
  		else //main stream
  		{
            requestedSize = 600*1024;
  		}

        __INFO("tcp stream, set send buffer to %d Bytes, sock = %d CameraIndex = %d StreamIndex = %d\n",
               requestedSize, fClientInputSocket, fCameraIndex, fStreamIndex); 
        increaseSendBufferTo(envir(), fClientInputSocket, requestedSize);
                
        //makeSocketTcpNoDelay(fClientInputSocket);
  	}
...
::handleCmd_SET_PARAMETER(ServerMediaSubsession* /*subsession*/, char const* cseq,
  ...
  if (strstr(fullRequestStr, "Request:Force IFrame"))
  {
    /* 强制I帧 */
    anj_video_request_idr(fCameraIndex, fStreamIndex);
  }
...
::authenticationOK(char const* cmdName, char const* cseq,
  ...
  //check if we have username and password from rtsp url
  char str_user[256]="";
  char str_pass[256]="";
  char *puser=strstr(urlSuffix,"?username=");
  char *ppass=strstr(urlSuffix,"&password=");
  
  if(puser && ppass)
  {
  	strncpy(str_user, puser + strlen("?username="), ppass - puser - strlen("?username="));
  	strcpy(str_pass, ppass + strlen("&password="));
  	
  	__INFO("user: [%s], pass: [%s]\n", str_user, str_pass); 
  	
 	const char *mypass = fOurServer.fAuthDB->lookupPassword(str_user);
 	if(mypass == NULL)
 		return False;
 	if(!strcmp(mypass, str_pass))
 	{
 		delete [] mypass;
 		return True;
 	}
 	...
 	{
    	char md5_pass[40];
		our_md5_encode(md5_pass, (const unsigned char *)mypass, strlen(mypass));
		delete [] mypass;
		
		if(!strcmp(md5_pass, str_pass))
		{
			return True;
		}
	    else
  ...
  do {
    ...
    password = fOurServer.fAuthDB->lookupPassword(username);
    
    __INFO("lookupPassword(%s) returned password %s\n", username, password);
    
    if (password == NULL) break;
    
    fCurrentAuthenticator.setUsernameAndPassword(username, password, fOurServer.fAuthDB->passwordsAreMD5());
    ...
    __INFO("ourResponse:%s\nresponse:%s\n", ourResponse, response);
    success = (strcmp(ourResponse, response) == 0);
    fCurrentAuthenticator.reclaimDigestResponse(ourResponse);
    
    if(!success) //check md5 password
    {
    	char md5_pass[40];    		

	   	our_md5_encode(md5_pass, (const unsigned char *)password, strlen(password));
	   	__ERR("password check failed, try check md5 password again: [%s]\n", md5_pass);
	   	    	
		fCurrentAuthenticator.setUsernameAndPassword(username, md5_pass, fOurServer.fAuthDB->passwordsAreMD5());
		// Finally, compute a digest response from the information that we have,
		// and compare it to the one that we were given:
		char const* md5Response	= fCurrentAuthenticator.computeDigestResponse(cmdName, uri);
		__INFO("ourResponse:%s\nresponse:%s\n", ourResponse, response);
		success = (strcmp(md5Response, response) == 0);
		fCurrentAuthenticator.reclaimDigestResponse(md5Response);
		
	    if(!success) //check md5 password
	    {
	    	char md5md5_pass[40];
		   	our_md5_encode(md5md5_pass, (const unsigned char *)md5_pass, strlen(md5_pass));
		   	__ERR("password check failed, try check md5md5 password again: [%s]\n", md5md5_pass);
		   	    	
			fCurrentAuthenticator.setUsernameAndPassword(username, md5md5_pass,fOurServer.fAuthDB->passwordsAreMD5());
			
			// Finally, compute a digest response from the information that we have,
			// and compare it to the one that we were given:
			char const* md5md5Response	= fCurrentAuthenticator.computeDigestResponse(cmdName, uri);
			success = (strcmp(md5md5Response, response) == 0);
			fCurrentAuthenticator.reclaimDigestResponse(md5md5Response);
	    }		
    }        
  } while (0);
...
void RTSPServer::RTSPClientSession::noteLiveness() {
  ...
  //__EVENT("Liveness indication from client at %s fClientOutputSocket %d\n",
          //our_inet_ntoa(fClientAddr.sin_addr), fClientOutputSocket);
  //__ERR("Liveness indication from client at %s\n", our_inet_ntoa(fClientAddr.sin_addr));
...
::livenessTimeoutTask(RTSPClientSession* clientSession) {
  ...
  if (NULL != clientSession)
  {
      __INFO("RTSP client session from %s has timed out fClientOutputSocket %d(due to inactivity)\n",
             our_inet_ntoa(clientSession->fClientAddr.sin_addr), clientSession->fClientOutputSocket);
  }
...
{
    char password[ACCOUNT_PASSWORD_MAX_LEN] = {0};

    int i = 0;
    int ifind = 0;
    SystemConfig *systemcfg = (SystemConfig *)getSystemConfig();
    UserConfig *usrcfg = &systemcfg->userCfg;

    for (i = 0; i < usrcfg->count; i++)
    {
        if (strcmp(username, usrcfg->accounts[i].userName) == 0)
        {
            if (strcmp("Enable", usrcfg->accounts[i].status) == 0)
            {
                ifind = 1;
                snprintf(password, sizeof(password), "%s", usrcfg->accounts[i].password);
                break;
            }
            else
            {
                __ERR("account: %s is disabled. status:%s\n", username, usrcfg->accounts[i].status);
            }
        }
    }
    ...
    if (1 == ifind)
    {
        __INFO("password=%s for user: %s\n",password, username);
        return (const char *)strdup(password);
    }
    else
    {
        __INFO("password not found for user: %s\n", username);
