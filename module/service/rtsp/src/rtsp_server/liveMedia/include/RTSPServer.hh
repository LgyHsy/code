...
public:
  static RTSPServer* createNew(UsageEnvironment& env, Port ourPort = 554,
		           ...
		           unsigned MaxClientSocketNum = 32);
  ...
  char* rtspOtherURL(ServerMediaSession const* serverMediaSession, int clientSocket, const char *otherName) const;
...
protected:
  ...
  public:
    ...
    static void deleteClientSession(RTSPClientSession* clientSession);    
  ...
  protected:
    ...
    virtual void handleCmd_PLAY(ServerMediaSubsession* subsession,
				char const* cseq, char const* fullRequestStr,char const *urlSuffix);
  ...
  protected:
    ...
    Boolean fClientSessionDeleting, fIsMulticast, fSessionIsActive, fStreamAfterSETUP;
    Authenticator fCurrentAuthenticator; // used if access control is needed
    unsigned char fTCPStreamIdCount; // used for (optional) RTP/TCP
    unsigned char fSocketRecvZero;
    unsigned char fSocketAuthPass;
    int fStreamIndex;
    int fCameraIndex;
...
private:
  ...
  unsigned fMaxClientSocketNum;  
  unsigned fServerRecvClientSocketNum;
  std::list<RTSPClientSession*> fClientSessionList;
};
