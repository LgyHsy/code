...
public:
  ...
  void modifySubSessionMediaPara(int iWidth, int iHeight, 
                                 int iFrameRate, int iBitRate);

  char const* streamName() const { return fStreamName; }
  Boolean checkStreamName(char const* streamName);
...
private:
  ...
  char fOtherStreamName[5][64];
public:
  int SetOtherStreamName(int index, char *streamName);
  int CheckOtherStreamName(const char *streamName);
  const char *GetOtherStreamName(int index);
};
...
public:
  ...
  virtual void sdpFree() = 0;
  virtual void modifyMediaPara(int iWidth, int iHeight, 
                               int iFrameRate, int iBitRate) = 0;
