...
public:
  ...
  int bufferData(unsigned char *buf, int size);
  int sendBuffer(void);
  int checkSendTimeout(int countMs);
  void updateSendTime(void);
  void updateBufferCheckTime(void);
  int checkBufferTimeout(int countMs) ;
...
public:
  ...
  unsigned char *fSendBuf;
  int  fSendBufPos;
  int  fSendBufOffset;
  int  fSendBufSize;
  struct timeval fLastSentTime;
  struct timeval fLastBufferCheckTime;  
};
...
public:
  ...
  int sendRTPOverTCP(unsigned char* packet, unsigned packetSize,tcpStreamRecord* stream);
