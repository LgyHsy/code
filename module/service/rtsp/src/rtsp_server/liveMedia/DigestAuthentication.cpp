...
void Authenticator::setRealmAndRandomNonce(char const* realm) {
  ...
  char nonceBuf[33];
  out_md5_encode_data((unsigned char*)(&seedData), sizeof(seedData), nonceBuf);
...
char const* Authenticator::computeDigestResponse(char const* cmd,
  ...
  if (fPasswordIsMD5) {
    ...
    out_md5_encode_data(ha1Data, ha1DataLen, ha1Buf);
    delete[] ha1Data;
  }
  ...
  char ha2Buf[33];
  out_md5_encode_data(ha2Data, ha2DataLen, ha2Buf);
  delete[] ha2Data;
  ...
  sprintf((char*)digestData, "%s:%s:%s",
          ha1Buf, nonce(), ha2Buf);
  char const* result = out_md5_encode_data(digestData, digestDataLen, NULL);
  delete[] digestData;
  return result;
}
