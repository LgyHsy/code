...
RTPSink::RTPSink(UsageEnvironment& env,
  ...
  u_int32_t timestampIncrement = (rtpTimestampFrequency*fCreationTime.tv_sec);
  timestampIncrement += (u_int32_t)((2.0*fTimestampFrequency*fCreationTime.tv_usec + 1000000.0)/2000000);
  
  fTimestampBase = timestampIncrement; //our_random32();
...
char* RTPSink::rtpmapLine() const {
  if ((rtpPayloadType() >= 96)||(rtpPayloadType() == 0)) 
  { // the payload format type is dynamic and PCMU
...
RTPTransmissionStats::RTPTransmissionStats(RTPSink& rtpSink, u_int32_t SSRC)
  ...
  SystemGetTimeofRun(&fTimeCreated, NULL);
...
::noteIncomingRR(struct sockaddr_in const& lastFromAddress,
  ...
  SystemGetTimeofRun(&fTimeReceived, NULL);
