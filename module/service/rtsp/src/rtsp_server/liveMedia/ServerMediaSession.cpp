...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
  ...
  strcpy(fOtherStreamName[0],"");
  strcpy(fOtherStreamName[1],"");
  strcpy(fOtherStreamName[2],"");
  strcpy(fOtherStreamName[3],"");
  strcpy(fOtherStreamName[4],"");
  
  SystemGetTimeofRun(&fCreationTime, NULL);
}

int ServerMediaSession::SetOtherStreamName(int index, char *streamName)
{
	if(index < 0 || index > 4)
		return -1;
	
	strcpy(fOtherStreamName[index], streamName);
	return 0;
}

int ServerMediaSession::CheckOtherStreamName(const char *streamName)
{
	int i;
	for(i=0;i<5;i++)
	{
		if(!strcmp(streamName, fOtherStreamName[i]))
			return 1;
	}
...
const char *ServerMediaSession::GetOtherStreamName(int index)
{
	if(index < 0 || index > 4)
		return "";
	
	return fOtherStreamName[index];	
}
...
Boolean ServerMediaSession::checkStreamName(char const* streamName)
{
    if (  (strncmp(streamName, fStreamName, strlen(streamName)) == 0)
        ||(strncmp(streamName, fOtherStreamName[0], strlen(streamName)) == 0)
        ||(strncmp(streamName, fOtherStreamName[1], strlen(streamName)) == 0)
        ||(strncmp(streamName, fOtherStreamName[2], strlen(streamName)) == 0)
        ||(strncmp(streamName, fOtherStreamName[3], strlen(streamName)) == 0)
        ||(strncmp(streamName, fOtherStreamName[4], strlen(streamName)) == 0) )
    {
        return True;
    }
...
void ServerMediaSession::modifySubSessionMediaPara(int iWidth, int iHeight, 
                                                   int iFrameRate, int iBitRate)
{
    ...
    for (subsession = fSubsessionsHead; subsession != NULL;
	     subsession = subsession->fNext) 
    {
        subsession->modifyMediaPara(iWidth, iHeight, iFrameRate, iBitRate);
        subsession->sdpFree();
    } 
