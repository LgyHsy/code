...
int iw_get_mac(int skfd, const char *ifname, char *mac)
{
  ...
  {
    iw_sawap_ntop(&(wrq.u.ap_addr), mac);
  }
  else
  {
    return (-1);
                         ...
                         double freq)
{
  ...
  else
  {
    ...
    snprintf(buffer, buflen, "%.3f %cHz", freq / divisor, scale);
  }
}
...
static inline void iw_get_ie_wpa(struct wireless_scan *wscan, unsigned char *iebuf,
                                 int buflen)
{
  int ielen = iebuf[1] + 2;
  int offset = 2; /* Skip the IE id, and the length. */
  ...
  case 0x30: /* WPA2 */
    ...
    {
      //  iw_print_ie_unknown(iebuf, buflen);
      // printf("unknown ie 444444444444\n");
      return;
    }
  ...
  if (iebuf[0] == 0xdd)
  {
    wscan->auth_info[wscan->auth_cnt].auth_type = 1;
    //	printf("WPA Version %d\n", ver);
  }
  if (iebuf[0] == 0x30)
  {
    wscan->auth_info[wscan->auth_cnt].auth_type = 2;
    //	printf("IEEE 802.11i/WPA2 Version %d\n", ver);
  }
  /* From here, everything is technically optional. */
  ...
  if (ielen < (offset + 4))
  {
    ...
    //  printf("						  Group Cipher : TKIP\n");
    //  printf("						  Pairwise Cipher : TKIP\n");
    wscan->auth_info[wscan->auth_cnt].group_cnt = 1;
    wscan->auth_info[wscan->auth_cnt].group_cipher[0] = IW_IE_CIPHER_TKIP;

    wscan->auth_info[wscan->auth_cnt].pairwise_cnt = 1;
    wscan->auth_info[wscan->auth_cnt].pairwise_cipher[0] = IW_IE_CIPHER_TKIP;
  ...
  if (memcmp(&iebuf[offset], wpa_oui, 3) != 0)
  {
    //	  printf("						  Group Cipher : Proprietary\n");
    wscan->auth_info[wscan->auth_cnt].group_cnt = 1;
    wscan->auth_info[wscan->auth_cnt].group_cipher[0] = IW_IE_CIPHER_NONE;
  }
  else
  {
    //  printf("						  Group Cipher :");
    ...
    //  printf("\n");
    wscan->auth_info[wscan->auth_cnt].group_cnt = 1;
    wscan->auth_info[wscan->auth_cnt].group_cipher[0] = iebuf[offset + 3];
  }
  offset += 4;
  ...
  wscan->auth_info[wscan->auth_cnt].pairwise_cnt = cnt;
  ...
  for (i = 0; i < cnt; i++)
  {
    if (memcmp(&iebuf[offset], wpa_oui, 3) != 0)
    {
      wscan->auth_info[wscan->auth_cnt].pairwise_cipher[i] = IW_IE_CIPHER_NONE;
    ...
    else
    {
      //  iw_print_value_name(iebuf[offset+3],
      //		  iw_ie_cypher_name, IW_IE_CYPHER_NUM);
      wscan->auth_info[wscan->auth_cnt].pairwise_cipher[i] = iebuf[offset + 3];
    }
    offset += 4;
  }
  //  printf("\n");
  ...
  wscan->auth_info[wscan->auth_cnt].key_type_cnt = cnt;
  ...
  for (i = 0; i < cnt; i++)
  {
    ...
    {
      //  printf(" Proprietary");
      wscan->auth_info[wscan->auth_cnt].key_type[i] = 0;
    }
    else
    {
      //  iw_print_value_name(iebuf[offset+3],
      //	  iw_ie_key_mgmt_name, IW_IE_KEY_MGMT_NUM);
      wscan->auth_info[wscan->auth_cnt].key_type[i] = iebuf[offset + 3];
    }
    offset += 4;
  }
  //	  printf("\n");
...
static inline void iw_gen_ie_info(struct wireless_scan *wscan, unsigned char *buffer, int buflen)
{
  int offset = 0;
  ...
  while (offset <= (buflen - 2))
  {
    ...
    case 0x30: /* WPA2 */
               //	  iw_print_ie_wpa(buffer + offset, buflen);
      iw_get_ie_wpa(wscan, buffer + offset, buflen);
      wscan->auth_cnt++;

      break;
    default:
      // printf("unknown ie 111111 \n");
      //  iw_print_ie_unknown(buffer + offset, buflen);
      break;
    }
    /* Skip over this IE to the next one in the list. */
...
void iw_get_link_stats(const iwqual *qual,
                       const iwrange *range,
                       int has_range, int *quality, int *signal_level, int *noise_level)
{
                           ...
                           const char *ifname)
{
  ...
  if ((ioctl(skfd, SIOCGIFHWADDR, &ifr) < 0) ||
      ((ifr.ifr_hwaddr.sa_family != ARPHRD_ETHER) && (ifr.ifr_hwaddr.sa_family != ARPHRD_IEEE80211)))
  {
    /* Deep trouble... */
                            ...
                            int we_version)
{
...
#ifdef DEBUG
  printf("__ERR - stream->current = %p, stream->value = %p, stream->end = %p\n",
...
#ifdef DEBUG
  printf("__ERR - iwe->cmd = 0x%X, iwe->len = %d\n",
         iwe->cmd, iwe->len);
#endif
...
#ifdef DEBUG
  printf("__ERR - event_type = %d, event_len = %d, pointer = %p\n",
    ...
    if (extra_len > 0)
    {
      ...
      else
      {
        ...
        if (iwe->u.data.length < descr->min_tokens)
          iwe->u.data.pointer = NULL; /* Discard paylod */
#ifdef DEBUG
        printf("__ERR - extra_len = %d, token_len = %d, token = %d, max = %d, min = %d\n",
  ...
  else
  {
     ...
     * Fixing that in the kernel would break 64 bits userspace. */
    if ((stream->value == NULL) && ((((iwe->len - IW_EV_LCP_PK_LEN) % event_len) == 4) || ((iwe->len == 12) && ((event_type == IW_HEADER_TYPE_UINT) ||
                                                                                                                (event_type == IW_HEADER_TYPE_QUAL)))))
    {
#ifdef DEBUG
      printf("__ERR - alt iwe->len = %d\n", iwe->len - 4);
#endif
      pointer -= event_len;
      pointer += 4;
      /* Beware of alignement. Dest has local alignement, not packed */
                          ...
                          int has_range)
{
  ...
  case SIOCGIWAP:
    ...
    wscan->has_ap_addr = 1;
    iw_saether_ntop(&event->u.ap_addr, wscan->mac);
  ...
  case SIOCGIWFREQ:
    ...
    wscan->b.channel = iw_freq_to_channel(wscan->b.freq, iw_range);
    break;
  case SIOCGIWMODE:
  ...
  // add by huangjm
  case SIOCGIWNAME:
    strcpy(wscan->b.name, event->u.name);
  ...
  case IWEVGENIE:
    ...
    iw_gen_ie_info(wscan, event->u.data.pointer, event->u.data.length);
  ...
  case SIOCGIWESSID:
    ...
    if ((event->u.essid.pointer) && (event->u.essid.length))
    {
      ...
      wscan->b.essid_len = event->u.essid.length;
    }
  ...
  case SIOCGIWENCODE:
    ...
    //   printf("keysize:%d, key_flag:%d, data:%p\n", wscan->b.key_size, wscan->b.key_flags, event->u.data.pointer);
  ...
  case IWEVQUAL:
    ...
    iw_get_link_stats(&wscan->stats.qual, iw_range, has_range, &quality, &signal_level, &noise_level);
    wscan->quality = quality;
    wscan->signal_level = signal_level;
    wscan->noise_level = noise_level;
    wscan->maxquality = iw_range->max_qual.qual;
  ...
  if (wrq.u.data.length)
  {
    ...
    do
    {
      ...
      if (ret > 0)
      {
        ...
        wscan = iw_process_scanning_token(&iwe, wscan, iw_range, has_range);
        /* Check problems */
        if (wscan == NULL)
        {
          free(buffer);
            ...
            wireless_scan_head *context)
{
  ...
  while (1)
  {
    ...
    delay = iw_process_scan(skfd, ifname, we_version, context, &range, has_range);
