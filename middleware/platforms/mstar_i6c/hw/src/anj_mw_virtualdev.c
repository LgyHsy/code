...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_mode", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", pwm_id, mode);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_period", VIRDEV_MOTOR_SYSFS_PATH);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_begin", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", pwm_id, begin);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_end", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", pwm_id, end);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_polarity", VIRDEV_MOTOR_SYSFS_PATH);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_round", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", group_id, step);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_enable", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", group_id, enable);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_round_get", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", group_id, 0);
...
{
    char path[64] = {0};
    snprintf(path, sizeof(path), "%sgroup_stop", VIRDEV_MOTOR_SYSFS_PATH);

    char buf[32] = {0};
    snprintf(buf, sizeof(buf), "%d %d", group_id, stop_flag);
...
@返回值： 成功返回0，失败返回-1
******************************************************************************/
static int anj_virdev_motor_duty_cycle_get(int pwm_num, int dir, int period, int *begin, int *end)
{
    switch (pwm_num)
    {
    case MOTO_PWM_ID0:
...
static void anj_virdev_motor_pwm_init(int pwm_num, int period, int dir)
{
    int begin = 0;
    int end = 0;
    anj_virdev_motor_set_group_mode(pwm_num, 1);
    if (pwm_num == MOTO_PWM_ID3)
    {
    ...
    anj_virdev_motor_duty_cycle_get(pwm_num, dir, period, &begin, &end);
    anj_virdev_motor_set_group_begin(pwm_num, begin);
    anj_virdev_motor_set_group_end(pwm_num, end);
}

void anj_virdev_motor_init()
{
    anj_mw_system("riu_w 0x101E 0x3 0");
...
int anj_virdev_motor_get_group_round()
{
    ...
    snprintf(path, sizeof(path), "cat %sgroup_round_get | grep PWM%d |awk '{print $4}'", VIRDEV_MOTOR_SYSFS_PATH, MOTO_PWM_ID0);
    FILE *fp = popen(path, "r");
    if (fp == NULL)
    {
        __ERR("popen return NULL buf:%d\n", path);
        return -1;
    }
    else
    {
...
int anj_virdev_motor_stop()
{
    ...
    if (s_stMotorRunParam.bRun)
    {
        ...
        iRunTotal = iRunStep + MOTOR_WAIT_STOP_STEP;
