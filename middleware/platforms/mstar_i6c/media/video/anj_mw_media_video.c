...
static int anj_mw_media_video_attr_init(AnjVideoConfig *pstAnjVideoCfg)
{
    ...
    for (iCameraIdex = 0; iCameraIdex < ANJ_CAMERA_MAX_NUMS; iCameraIdex++)
    {
        ...
        gstVideoAttr[iCameraIdex].IspAttr.ePixelFormat =
            (ANJ_CAMERA_MAX_NUMS > 1) ? E_MI_SYS_PIXEL_FRAME_YUV_SEMIPLANAR_420 : E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
        ...
        for (int i = 0; i < MAX_VENC_CHN; i++)
        {
            ...
            __ERR("qp:%d, %d set to cfg qp:%d, %d\n",
...
int anj_mw_media_video_init(AnjVideoConfig *pstAnjVideoCfg)
{
    ...
    MI_SYS_CompressMode_e eCompressMode = E_MI_SYS_COMPRESS_MODE_IFC;
    ...
    MI_SYS_WindowSize_t stSnrRes;
    memset(&stSnrRes, 0, sizeof(MI_SYS_WindowSize_t));
    ...
    for (iCameraIdex = 0; iCameraIdex < ANJ_CAMERA_MAX_NUMS; iCameraIdex++)
    {
        ...
        STCHECKRESULT(ST_Common_SensorInit(gstVideoAttr[iCameraIdex].SnrPadId, pSclAttr->u32SrcFrmRateNum));
    ...
    for (iCameraIdex = 0; iCameraIdex < ANJ_CAMERA_MAX_NUMS; iCameraIdex++)
    {
        for (int i = 0; i < MAX_VENC_CHN; i++)
        {
            ...
            s_stVencStreamThread[VencChnId].bAutoDestroy = 1;
