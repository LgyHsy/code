...
MI_S32 ST_Common_SensorInit(MI_SNR_PADID eSnrPad, MI_U32 u32Fps)
{
    ...
    *************************************************/
    ...
    __INFO("sensor(%d)(max:%d): \n", eSnrPadId, u32ResCount - 1);

    STCHECKRESULT(MI_SNR_SetRes(eSnrPadId, 0));

    STCHECKRESULT(MI_SNR_GetRes(eSnrPadId, 0, &stRes));
...
MI_S32 ST_Common_SensorGetRectInfo(MI_SNR_PADID eSnrPad, MI_SYS_WindowSize_t *pstSnrRes)
{
    MI_U32 u32PlaneId = 0;
    MI_SNR_PlaneInfo_t stSnrPlane0Info;
    memset(&stSnrPlane0Info, 0x0, sizeof(MI_SNR_PlaneInfo_t));
    ...
    pstSnrRes->u16Width = stSnrPlane0Info.stCapRect.u16Width;
    pstSnrRes->u16Height = stSnrPlane0Info.stCapRect.u16Height;
...
{
    STCHECKRESULT(MI_SNR_SetOrien(eSnrPadId, bHFlip, bVFlip));
    #if 0
    int times = 0;
    do 
    {
        ...
        __ERR("MI_SNR_SetOrien %d %d OK\n", bHFlip, bVFlip);
        
        MI_BOOL bMirror;
        MI_BOOL bFlip;
        
        STCHECKRESULT(MI_SNR_GetOrien(eSnrPadId, &bMirror, &bFlip));

        if (bMirror != bHFlip || bFlip != bVFlip)
        {
            __ERR("MI_SNR_GetOrien %d %d != setted %d %d\n", bMirror, bFlip, bHFlip, bVFlip);
        }
        else
        {
            __ERR("MI_SNR_GetOrien %d %d\n", bMirror, bFlip);
            break;
        }
