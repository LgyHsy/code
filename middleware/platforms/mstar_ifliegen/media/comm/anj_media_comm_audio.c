...
MI_S32 ST_Common_AudioAo_SetMute(MI_AUDIO_DEV AoDevId, int enable)
{
    MI_BOOL leftMute;
    MI_BOOL rightMute;
    leftMute = enable > 0 ? 1 : 0;
    rightMute = enable > 0 ? 1 : 0;

    STCHECKRESULT(MI_AO_SetMute(AoDevId, leftMute, rightMute, E_MI_AO_GAIN_FADING_OFF));
    return 0;
}
...
MI_S32 ST_Common_AudioAiInit(ST_Common_AudioAttr_t *pstAudioAttr)
{
    ...
    MI_S16 s8dpgaGain[] = {ST_AUDIO_AI_DPGA_GAIN};
