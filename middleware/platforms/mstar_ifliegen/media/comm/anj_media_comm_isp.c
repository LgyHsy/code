...
#define AIISP_CALI_PATH AJ_APP_PATH "/aibnr_cali_ok.data"
#define AIISP_MODEL_PATH AJ_APP_PATH "/bnr_high_2560x1440.img"

static int gstAiIspInit = 0;
static MI_U32 gstAiIspModelHighId = 0;
static int gstAiIspEnabled = 0;
...
MI_S32 ST_Common_IspStart(ST_Common_IspAttr_t *pstIspAttr, MI_SYS_WindowRect_t *pSnrRes)
{
    ...
    stIspChnParam.e3DNRLevel = E_MI_ISP_3DNR_LEVEL4;
    ...
    stIspOutputParam.eBufLayout = E_MI_ISP_BUFFER_LAYOUT_ONE_FRAME;
...
MI_S32 ST_Common_IspAiStart(ST_Common_IspAttr_t *pstIspAttr, int aiIspMode)
{
    ...
    if (gstAiIspInit == 0)
    {
        MI_ISP_AiBnrModelParam_t stAiBnrModelHighParam = {0};
        strncpy((char *)stAiBnrModelHighParam.u8ModelPath, AIISP_MODEL_PATH, sizeof(stAiBnrModelHighParam.u8ModelPath));
        stAiBnrModelHighParam.bUpdate = TRUE;

        STCHECKRESULT(MI_ISP_AiIspInit(pstIspAttr->IspDevId, pstIspAttr->IspChnId));
        STCHECKRESULT(MI_ISP_LoadAiBnrModel(pstIspAttr->IspDevId, pstIspAttr->IspChnId, &stAiBnrModelHighParam, &gstAiIspModelHighId));
        gstAiIspInit = 1;
    }

    // 默认极低亮度模型
    stAiBnrAttr.u32ModelId = gstAiIspModelHighId;
    if (aiIspMode)
    {
    ...
    STCHECKRESULT(MI_ISP_SetAiBnrAttr(pstIspAttr->IspDevId, pstIspAttr->IspChnId, &stAiBnrAttr));
    gstAiIspEnabled = aiIspMode ? 1 : 0;
    return MI_SUCCESS;
}
...
MI_S32 ST_Common_IspAiStop(ST_Common_IspAttr_t *pstIspAttr)
{
    ...
    {
        MI_ISP_AiBnrAttr_t stAiBnrAttr = {0};
        MI_ISP_GetAiBnrAttr(pstIspAttr->IspDevId, pstIspAttr->IspChnId, &stAiBnrAttr);
        if (stAiBnrAttr.eAiSourceType > E_MI_ISP_AI_BNR_SOURCE_TYPE_NONE)
        {
            stAiBnrAttr.eAiSourceType = E_MI_ISP_AI_BNR_SOURCE_TYPE_NONE;
        }
        STCHECKRESULT(MI_ISP_SetAiBnrAttr(pstIspAttr->IspDevId, pstIspAttr->IspChnId, &stAiBnrAttr));
        STCHECKRESULT(MI_ISP_UnloadAiBnrModel(pstIspAttr->IspDevId, pstIspAttr->IspChnId, gstAiIspModelHighId));
        gstAiIspInit = 0;
        gstAiIspEnabled = 0;
    }
    return MI_SUCCESS;
...
{
    MI_ISP_DEV IspDevId = MI_ISP_DEV0;
    CUS3A_ALGO_STATUS_t status;

    STCHECKRESULT(CUS3A_GetAlgoStatus(IspDevId, IspChnId, &status));

    if (E_ALGO_STATUS_RUNNING == status.Ae)
    {
        return MI_SUCCESS;
    }
...
{
    MI_S32 s32Ret = 0;
    s32Ret = MI_ISP_ApiCmdLoadCaliData(DevId, Channel, (MI_ISP_IQ_CaliItem_e)8, AIISP_CALI_PATH);
    return s32Ret;
}
...
MI_S32 ST_Common_IspFilckerSet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, int Hz)
{
    ...
    MI_ISP_AE_GetFlickerEx(DevId, Channel, &data);
    __INFO("Current Flicker Enable:%d, Type:%s\n", data.bEnable,
           data.eFlickerType == E_SS_AE_FLICKER_TYPE_DETECT_50HZ ? "50HZ" : "60HZ");
...
MI_S32 ST_Common_IspBLCGet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, char *backlight)
{
    ...
    {
        backlight[i] = data.stAuto.stParaAPI[i].u8Strength;
    }
    return MI_SUCCESS;
}
...
MI_S32 ST_Common_IspBLCSet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, char *oriBacklight, int backlight)
{
    ...
    if (backlight)
    {
        for (i = 0; i < ISP_BASE_PARAM_MAX_CHN_16; i++)
        {
            char oldvalue = data.stAuto.stParaAPI[i].u8Strength;
            char newvalue = backlight;
    ...
    else
    {
        for (i = 0; i < ISP_BASE_PARAM_MAX_CHN_16; i++)
        {
            if (data.stAuto.stParaAPI[i].u8Strength != oriBacklight[i])
            {
                chgflag = 1;
                data.stAuto.stParaAPI[i].u8Strength = oriBacklight[i];
            }
        }
    }
...
MI_S32 ST_Common_IspHLCSet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, int hlc, int brightness)
{
    ...
    {
        __INFO("HLC brightness path disabled, skip brightness compensation\n");
        return MI_SUCCESS;
    }
...
MI_S32 ST_Common_IspConvergeSpeedSet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, unsigned int *u32SpeedY)
{
    MI_ISP_AE_ConvConditonType_t ae_data = {0};
    STCHECKRESULT(MI_ISP_AE_GetConverge(DevId, Channel, &ae_data));
    if (ae_data.stConvSpeed.u32SpeedY[0] != u32SpeedY[0] ||
        ae_data.stConvSpeed.u32SpeedY[1] != u32SpeedY[1] ||
        ae_data.stConvSpeed.u32SpeedY[2] != u32SpeedY[2] ||
        ae_data.stConvSpeed.u32SpeedY[3] != u32SpeedY[3])
    {
        ae_data.stConvSpeed.u32SpeedY[0] = u32SpeedY[0];
        ae_data.stConvSpeed.u32SpeedY[1] = u32SpeedY[1];
        ae_data.stConvSpeed.u32SpeedY[2] = u32SpeedY[2];
        ae_data.stConvSpeed.u32SpeedY[3] = u32SpeedY[3];
...
{
    MI_ISP_StopPortZoom(DevId, Channel);
    return MI_SUCCESS;
}
...
{
    MI_ISP_IQ_TempInfoType_t tempInfo = {0};
    MI_ISP_IQ_GetTempInfo(DevId, Channel, &tempInfo);
    return tempInfo.u8Temperature;
}

MI_S32 ST_Common_IspSkipFrameSet(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel, int FrameCnt)
{
    STCHECKRESULT(MI_ISP_SkipFrame(DevId, Channel, FrameCnt));
    return MI_SUCCESS;
}

MI_S32 ST_Common_IspChannelStop(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel)
{
    STCHECKRESULT(MI_ISP_StopChannel(DevId, Channel));
    return MI_SUCCESS;
}

MI_S32 ST_Common_IspChannelStart(MI_ISP_DEV DevId, MI_ISP_CHANNEL Channel)
{
    STCHECKRESULT(MI_ISP_StartChannel(DevId, Channel));
    return MI_SUCCESS;
}
