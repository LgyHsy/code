...
MI_S32 ST_Common_SysInit(void)
{
    ...
    #if defined(ISP_AI_SUPPORT)
    	anj_mw_system("echo 400000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_min_freq");
    	anj_mw_system("echo 1000000 > /sys/devices/system/cpu/cpufreq/policy0/scaling_max_freq");
    	anj_mw_system("echo 600 > /proc/mi_modules/mi_ipu/debug_hal/freq");
    	anj_mw_system("echo 384000000 > /proc/mi_modules/mi_scl/debug_hal/clk");
    #endif
...
char *ST_Common_SysMmap(unsigned long long u64PhyAddr, unsigned int mapsize)
{
    ...
    {
        __ERR("phys_addr = %#llx, MI_SYS_FlushInvCache failed\n", u64PhyAddr);
