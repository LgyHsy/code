...
int anj_mw_osd_create(overlay_param_s *overlay_param, ANJ_SIZE_S *resolution, int rgn_pixel_format)
{
    ...
    else
    {
        ...
        m_stRgnPoint.s32X = overlay_param->pos_x;
        m_stRgnPoint.s32Y = overlay_param->pos_y;
    }
    ...
    stRgnChnPortParam.u32Layer =
        OSD_FULL_IMAGE ? (overlay_param->stream_type * OSD_TYPE_MAX) : overlay_param->rgn_handle;
    stRgnChnPortParam.stOsdChnPort.stPoint.s32X = m_stRgnPoint.s32X;
    stRgnChnPortParam.stOsdChnPort.stPoint.s32Y = m_stRgnPoint.s32Y;
    stRgnChnPortParam.stOsdChnPort.stOsdAlphaAttr.eAlphaMode = E_MI_RGN_PIXEL_ALPHA;
    stRgnChnPortParam.stOsdChnPort.stOsdAlphaAttr.stAlphaPara.stArgb1555Alpha.u8BgAlpha = 0x0;
    stRgnChnPortParam.stOsdChnPort.stOsdAlphaAttr.stAlphaPara.stArgb1555Alpha.u8FgAlpha = 0xAA;
...
int anj_mw_osd_destroy(overlay_param_s *overlay_param)
{
    ...
    m_stRgnChnPort.s32ChnId = overlay_param->stream_type;
    m_stRgnChnPort.s32PortId = 0;
    m_stRgnChnPort.bInputPort = FALSE;
...
int anj_mw_osd_canvas_update(overlay_param_s *overlay_param)
{
    ...
    else
    {
        ...
        if (stRgnAttr.stOsdInitParam.stSize.u32Width != overlay_param->width ||
            ...
            stChnPortAttr.stOsdChnPort.stPoint.s32X != overlay_param->pos_x ||
            stChnPortAttr.stOsdChnPort.stPoint.s32Y != overlay_param->pos_y)
        {
            // __INFO("Recreate RGN handle %d for size change %dx%d %dx%d -> %dx%d %dx%d\n",
            //        iRgnCanvasIndex,
            //        stChnPortAttr.stOsdChnPort.stPoint.s32X,
            //        stChnPortAttr.stOsdChnPort.stPoint.s32Y,
...
int anj_mw_osd_draw_overlay_create(int stream_type, int stream_width, int stream_height, int rgn_handle)
{
    ...
    memset(&stRgnChnPortParam, 0x00, sizeof(MI_RGN_ChnPortParam_t));
    stRgnChnPortParam.bShow = TRUE;
    stRgnChnPortParam.u32Layer = (MI_U32)u32OsdHandle;
    stRgnChnPortParam.stOsdChnPort.stPoint.s32X = m_stRgnPoint.s32X;
    stRgnChnPortParam.stOsdChnPort.stPoint.s32Y = m_stRgnPoint.s32Y;
    stRgnChnPortParam.stOsdChnPort.stOsdAlphaAttr.eAlphaMode = E_MI_RGN_PIXEL_ALPHA;
    stRgnChnPortParam.stOsdChnPort.stOsdAlphaAttr.stAlphaPara.stArgb1555Alpha.u8BgAlpha = 0x0;
...
int anj_mw_osd_cover_create(cover_param_s *cover_param)
{
    ...
    m_stRgnPoint.s32X = cover_param->cover_rect.pos_x;
    m_stRgnPoint.s32Y = cover_param->cover_rect.pos_y;
    ...
    if (m_stRgnSize.u32Width * m_stRgnSize.u32Height > 0)
    {
        ...
        stRgnChnPortParam.stCoverChnPort.eAreaType = E_MI_RGN_AREA_TYPE_RECT;
        stRgnChnPortParam.stCoverChnPort.stRect.s32X = m_stRgnPoint.s32X;
        stRgnChnPortParam.stCoverChnPort.stRect.s32Y = m_stRgnPoint.s32Y;
        stRgnChnPortParam.stCoverChnPort.stRect.u32Width = m_stRgnSize.u32Width;
        stRgnChnPortParam.stCoverChnPort.stRect.u32Height = m_stRgnSize.u32Height;
        stRgnChnPortParam.stCoverChnPort.eMode = E_MI_RGN_COVER_MODE_COLOR;
        stRgnChnPortParam.stCoverChnPort.stColorAttr.u32Color = cover_param->cover_rect.u32Color;
        ...
        __INFO("cover handle:%d xy:%d %d wd:%d %d create sucess!\n",
               u32OsdHandle, m_stRgnPoint.s32X, m_stRgnPoint.s32Y, m_stRgnSize.u32Width, m_stRgnSize.u32Height);
    }
